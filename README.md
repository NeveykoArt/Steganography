...
Программа реализует встраивание секретного сообщения в текст и последующее извлечение. Когда в функцию encode попадает текст, берется случайный индекс символа в строке, происходит сравнение с символом из словаря, и, если совпадение есть, то в случае 1 из секретного сообщения происходит замена этого символа на измененный символ, внешне похожий на оригинальный. Извлечение работает в обратную сторону.

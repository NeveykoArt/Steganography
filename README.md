...
# Лаба 6
Запуск: python3 lab6.py

Как работает?

Программа реализует подсчет ошибки первого и второго рода для методов стегоанализа. Результат работы - точечные диаграммы для каждого из методов. Перебираются все возможные пороговые значения для определения секретного сообщения в контейнере.


# Лаба 8
Запуск: python3 lab8.py

Как работает?

Программа реализует встраивание секуретного сообщения в текст и последующее извлечение. Когда в функцию encode попадает текст, берется случайный индекс символа в строке, происходит сравнение с символом из словаря, и, если совпадение есть, то в случаае 1 из секретного сообщения происходит замена этого символа на измененный символ, внешне похожий на оригинальный. Извлечение работает в обратную сторону.
